...
func _create_hitboxes() -> void:
	for child:Node in get_children():
		for mesh:MeshInstance3D in child.get_children():
			var base_mesh_name:String = mesh.name.lstrip(child.name)
			var name_parts:PackedStringArray = base_mesh_name.split("_")
			...
			if hitbox_as_is.has(mesh_name):
				...
				var hitbox_name:String = "%s%sHitbox%s" % [child.name, mesh_name, mesh_index]
				var hitbox_mesh:MeshInstance3D = child.find_child(hitbox_name)
				if not hitbox_mesh:
					continue
